...
output "vault_command" {
  value = "vault read -namespace=admin/${vault_namespace.demo.path_fq} ${vault_mount.database_mssql.path}/creds/${vault_database_secret_backend_connection.cloudsql.name}"
}
